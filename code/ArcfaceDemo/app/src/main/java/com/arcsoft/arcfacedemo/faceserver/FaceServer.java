...
import com.arcsoft.arcfacedemo.activity.SimpleService;
...
import com.arcsoft.arcfacedemo.util.MediaTypeUtil;
...
public class FaceServer {
     ...
     * @return 是否注册成功
     */
    public boolean registerToServer(Context context, byte[] nv21, int width, int height, String name, String server) {
        synchronized (this) {
            if (faceEngine == null || context == null || nv21 == null || width % 4 != 0 || nv21.length != width * height * 3 / 2) {
                return false;
            }
    ...
    public void writeToServer(File file, String server) {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(server)
                .addConverterFactory(GsonConverterFactory.create())
                .build();

        InterfaceServer interfaceServer = retrofit.create(InterfaceServer.class);
        ...
        String groupId = "101";
        RequestBody requestBodyGroupId =
                RequestBody.create(
        ...
        Call<ResponseBody> call = interfaceServer.register(body, requestBodyGroupId, requestBodyName);
     ...
     * @return 比对结果
     */
    public CompareResult getTopOfFaceLibFromServer(FaceFeature faceFeature, String server, Context context) {
        ...
        Call<FaceSearchResDto> call = interfaceServer.getTopOfFaceLibFromServer(new FaceRegisterInfo(faceFeature.getFeatureData(), 101, ""));

        // Fetch and print a list of the contributors to the library.
        FaceSearchResDto faceSearchResDto = new FaceSearchResDto();
        try {
            faceSearchResDto = call.execute().body();
        ...
        synchronized (this) {
            ...
            //2.特征提取
            // code = faceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfoList.get(0), faceFeature);
            String userName = faceSearchResDto.getFaceId() == null ? String.valueOf(System.currentTimeMillis()) : faceSearchResDto.getFaceId();
            try {
                //3.保存注册结果（注册图、特征数据）
                File file = new File(imgDir + File.separator + userName + IMG_SUFFIX);
                String face = faceSearchResDto.getFace();
                byte[] bytes = face.getBytes("ISO-8859-1");
                ...
                faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), faceSearchResDto.getGroupId(), userName));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }

        return getTopOfFaceLib(faceFeature);
    }
