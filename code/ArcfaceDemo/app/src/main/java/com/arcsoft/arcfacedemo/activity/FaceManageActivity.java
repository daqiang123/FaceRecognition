...
import com.arcsoft.arcfacedemo.faceserver.InterfaceServer;
...
public class FaceManageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Call<SimpleService.Contributor> call = github.contributors();

        // Fetch and print a list of the contributors to the library.
        try {
            SimpleService.Contributor contributors = call.execute().body();
            ...
            public void run() {
                ...
                for (int i = 0; i < totalCount; i++) {
                    ...
                    if ("1".equals(mode)) { // 1：客户端版
                        success = FaceServer.getInstance().register(FaceManageActivity.this, nv21, bitmap.getWidth(), bitmap.getHeight(),
                                jpgFile.getName().substring(0, jpgFile.getName().lastIndexOf(".")));
                    } else { // 2：服务器版
                        success = FaceServer.getInstance().registerToServer(FaceManageActivity.this, nv21, bitmap.getWidth(), bitmap.getHeight(),
                                jpgFile.getName().substring(0, jpgFile.getName().lastIndexOf(".")), server);
                    }
