...
import com.arcsoft.arcfacedemo.util.PropertiesUtil;
...
//                    Log.i(TAG, "onPreview: fr end = " + System.currentTimeMillis() + " trackId = " + requestId);
                    ...
                    //不做活体检测的情况，直接搜索
                    if (!livenessDetect) {
                        searchFace(faceFeature, requestId, mode, server);
                    }
                    //活体检测通过，搜索特征
                ...
                if (registerStatus == REGISTER_STATUS_READY && facePreviewInfoList != null && facePreviewInfoList.size() > 0) {
                    ...
                    Observable.create(new ObservableOnSubscribe<Boolean>() {
                        ...
                        public void subscribe(ObservableEmitter<Boolean> emitter) {
                            ...
                            boolean success;
                            if ("1".equals(mode)) { // 1：客户端版
                                success = FaceServer.getInstance().register(RegisterAndRecognizeActivity.this, nv21.clone(), previewSize.width, previewSize.height, "registered " + faceHelper.getCurrentTrackId());
                            } else { // 2：服务器版
                                success = FaceServer.getInstance().registerToServer(RegisterAndRecognizeActivity.this, nv21.clone(), previewSize.width, previewSize.height, "registered " + faceHelper.getCurrentTrackId(), server);
                            }
...
//                            Log.i(TAG, "onPreview: fr start = " + System.currentTimeMillis() + " trackId = " + facePreviewInfoList.get(i).getTrackId());
     ...
     * @param frFace
     * @param requestId
     * @param mode 模式（1：客户端版；2：服务器版）
     */
    private void searchFace(final FaceFeature frFace, final Integer requestId, final String mode, final String server) {
        Observable
...
//                      Log.i(TAG, "subscribe: fr search start = " + System.currentTimeMillis() + " trackId = " + requestId);
                        ...
                        if ("1".equals(mode)) { // 1：客户端版
                            compareResult = FaceServer.getInstance().getTopOfFaceLib(frFace);
                        } else { // 2：服务器版
                            // 先从本地搜索人脸
                            compareResult = FaceServer.getInstance().getTopOfFaceLib(frFace);
                            // 如果没有搜索到，再去服务器搜索。
                            if (compareResult == null) {
                                compareResult = FaceServer.getInstance().getTopOfFaceLibFromServer(frFace, server, getApplicationContext());
                            }
                        }
